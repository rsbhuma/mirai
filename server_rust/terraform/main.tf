# AWS Community Coin Server - Auto-scaling Infrastructure
...
terraform {
  ...
  # backend "s3" {
  #   bucket = "community-coin-terraform-state"
...
variable "domain_name" {
  ...
  type        = string
  default     = "api.communitycoin.com"
}

variable "min_capacity" {
  description = "Minimum number of containers"
  type        = number
  default     = 1  # Start with just 1 container
}

variable "max_capacity" {
  description = "Maximum number of containers"
  type        = number
  default     = 100  # Can scale up to 100 containers
}
...
variable "db_instance_class" {
  ...
  type        = string
  default     = "db.t4g.micro"  # Start small, can be upgraded
}
...
locals {
  ...
  # Cost optimization: Use only 2 AZs to reduce NAT Gateway costs
...
resource "aws_subnet" "private" {
  ...
  vpc_id            = aws_vpc.main.id
  cidr_block        = "10.0.${count.index * 10 + 2}.0/24"
  availability_zone = local.azs[count.index]
...
resource "aws_subnet" "database" {
  ...
  vpc_id            = aws_vpc.main.id
  cidr_block        = "10.0.${count.index * 10 + 3}.0/24"
  availability_zone = local.azs[count.index]
